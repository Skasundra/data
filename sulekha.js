...
const searchSulekha = async (req, res) => {
  let page = null;
  const startTime = Date.now();

  try {
    console.log("Sulekha request received:", JSON.stringify(req.body));
    ...
    while (allBusinesses.length < maxResults && currentPage <= maxPages) {
      ...
      // Sulekha URL format: https://www.sulekha.com/{city}/{keyword}?page={page}
      const citySlug = place.toLowerCase().replace(/\s+/g, "-");
      const keywordSlug = keyword.toLowerCase().replace(/\s+/g, "-");
      
      const pageUrl =
        currentPage === 1
          ? `https://www.sulekha.com/${citySlug}/${keywordSlug}`
          : `https://www.sulekha.com/${citySlug}/${keywordSlug}?page=${currentPage}`;
      ...
      businessElements.each((index, el) => {
        ...
        try {
          ...
          // Extract Sulekha URL
          const sulekhaPath = $el.find("a").first().attr("href");
          const sulekhaUrl = sulekhaPath
            ? sulekhaPath.startsWith("http")
              ? sulekhaPath
              : `https://www.sulekha.com${sulekhaPath}`
            : "";

          // Extract category
          let category = "";
          const categoryEl = $el.find('[class*="category"]').first();
          ...
          const businessData = {
            userId,
            businessId: `sulekha_${storeName.toLowerCase().replace(/[^a-z0-9]/g, "_")}`,
            storeName,
            ...
            stars,
            numberOfReviews,
            ratingText: stars ? `${stars} stars with ${numberOfReviews || 0} reviews` : "",
          ...
          console.log("Error parsing Sulekha business data:", parseError.message);
        }
      });
    ...
    console.log(`Sulekha scraping completed in ${executionTime} seconds`);

    return res.status(200).json({
      status: 200,
      message: "Sulekha leads generated successfully.",
      data: allBusinesses,
      metadata: {
    ...
    console.error("Error in searchSulekha:", error.message);
