...
const searchExportersIndia = async (req, res) => {
  let page = null;
  const startTime = Date.now();

  try {
    console.log("ExportersIndia request received:", JSON.stringify(req.body));
    ...
    while (allBusinesses.length < maxResults && currentPage <= maxPages) {
      ...
      // ExportersIndia URL format: https://www.exportersindia.com/search/{keyword}.htm?city={city}&page={page}
      const pageUrl =
        currentPage === 1
          ? `https://www.exportersindia.com/search/${encodeURIComponent(keyword)}.htm?city=${encodeURIComponent(place)}`
          : `https://www.exportersindia.com/search/${encodeURIComponent(keyword)}.htm?city=${encodeURIComponent(place)}&page=${currentPage}`;
      ...
      businessElements.each((index, el) => {
        ...
        try {
          ...
          // Extract ExportersIndia URL
          const eiPath = $el.find("a").first().attr("href");
          const eiUrl = eiPath
            ? eiPath.startsWith("http")
              ? eiPath
              : `https://www.exportersindia.com${eiPath}`
            : "";
          ...
          const businessType = $el.find('[class*="business-type"]').text().trim() || "";

          const businessData = {
            userId,
            businessId: `exportersindia_${storeName.toLowerCase().replace(/[^a-z0-9]/g, "_")}`,
            storeName,
            category: products.join(", ") || "Business",
            address: address || "",
            phone: phones.join(", ") || "",
            exportersindiaUrl: eiUrl,
            bizWebsite: "",
            trustSeal,
            gstNumber,
            businessType,
          ...
          console.log("Error parsing ExportersIndia business data:", parseError.message);
        }
      });
    ...
    console.log(`ExportersIndia scraping completed in ${executionTime} seconds`);

    return res.status(200).json({
      status: 200,
      message: "ExportersIndia leads generated successfully.",
      data: allBusinesses,
      metadata: {
    ...
    console.error("Error in searchExportersIndia:", error.message);
...
module.exports = { searchExportersIndia };
